	...
	public static void execute(LevelAccessor world, Entity entity) {
		...
		if (!entity.getPersistentData().getBoolean("voreSetup")) {
			...
			if ((entity.getPersistentData().getString("bellyType")).isEmpty()) {
				if (entity.getType().is(TagKey.create(Registries.ENTITY_TYPE, new ResourceLocation("forge:stomach_noeat")))) {
					entity.getPersistentData().putString("bellyType", "noeat");
					entity.getPersistentData().putBoolean("noEat", true);
				} else if (entity.getType().is(TagKey.create(Registries.ENTITY_TYPE, new ResourceLocation("forge:stomach_animal")))) {
					entity.getPersistentData().putString("bellyType", "animal");
				} else if (entity.getType().is(TagKey.create(Registries.ENTITY_TYPE, new ResourceLocation("forge:stomach_anthro"))) || entity instanceof Player) {
					entity.getPersistentData().putString("bellyType", "anthro");
				} else if (entity.getType().is(TagKey.create(Registries.ENTITY_TYPE, new ResourceLocation("forge:stomach_rotten")))) {
					entity.getPersistentData().putString("bellyType", "rotten");
				} else if (entity.getType().is(TagKey.create(Registries.ENTITY_TYPE, new ResourceLocation("forge:stomach_rotten_animal")))) {
				...
				if (world.getLevelData().getGameRules().getBoolean(VoreModModGameRules.VOREBOSE)) {
					VoreModMod.LOGGER.info(entity.getDisplayName().getString() + " got assigned belly name of " + entity.getPersistentData().getString("bellyType"));
				}
				if ((entity.getPersistentData().getString("bellyType")).equals("animal") || (entity.getPersistentData().getString("bellyType")).equals("rotten_animal")) {
